...
from qdb.fixtures import orchestrator
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        try:
            orchestrator1 = orchestrator.Orchestrator(DB_FILE, REPORTS_DIR, DEFAULT_RECIPIENTS)
            if list_units is True:
                print(orchestrator1.list_units())
                exit(0)
            yyyymm = orchestrator1.validate_date(year, month, yyyymm=True)
            units = orchestrator1.validate_units(units)
            orchestrator1.run(yyyymm, units, send_email=email,
